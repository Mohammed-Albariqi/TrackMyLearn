...
import { set, db, ref, update } from "../../services/firebase";
    ...
    return (
      <OutlinedButton
        ...
        onClick={async () => {
          ...
          const currentUser = userData.find((user) => user.uid === uid);
          if (currentUser && currentUser.type === "instructor") {
            // Send data to Firebase
            set(ref(db, `courses/${classroom}/report`), userData);
            update(ref(db, `courses/${classroom}/`), {
              activeReport: true,
            });
            // End the meeting
